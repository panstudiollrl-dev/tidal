...
- Vendor ID: `0x08E2`
- Product ID: `0x0101`
- 目前使用中的球仍以 WebHID 授權到的 `MB01` 為準；瀏覽器頁面不依賴固定 serial。

WebHID 不會穩定把 serial number 暴露給頁面，所以程式用 vendor/product 來篩選，並依連線順序分配 Ball 1 / Ball 2。若換同型 `MB01`，通常不需要改 `SUPPORTED_GRIP_BALLS`；換球後若舊授權開不起來，`syncBalls()` 只在 `open()` 失敗時撤銷那顆授權，避免 watchdog 反覆把真球清掉。
...
11. 除錯操作：**D** 開關右上診斷面板；**L** 下載最近真球操作 log（`tidal_grip_operation_log.json`，也存於 `localStorage` 的 `tidal_grip_operation_log_v1`）；**R** 撤銷並重新配對握力球。
...
- **握力水位映射最新狀態（Codex 2026-07-20）**：目前程式不是穩定結論，只是為了蒐集真球資料而加了 log 與 polarity 觀察。接手者請以 `web/index.html` 為準：
  - `GRIP_MIN_SPAN = 250`：曾嘗試降低 span floor 讓弱球可達到明顯水位，但 Pan 回饋仍不穩，可能導致某些球低端過敏。
  - `GRIP_HEADROOM = 1.35`（原 1.0）：舒適最大握力不會立刻等於 100% 滿水位。
  - `GRIP_GAMMA = 0.78`（原 0.55）：低-中握不再被過度放大。
  - `GRIP_DEADZONE = 0.10`：把「拿起球／手指貼著」留在 0 附近。
  - `GRIP_POLARITY_MARGIN = 45` 與 per-ball `sign`：校正 cue 期間收集 rest/press median，以 `press-rest` 判斷該球是 raw 上升＝握或 raw 下降＝握。這個方向目前仍可能被拿球姿勢、baseline 漂移或弱訊號污染。
  - `GripCalibrator` 目前仍以單一 `level` 供 Arrival 與 4-7-8 使用。Pan 最新回饋表示 4-7-8 仍會卡住，代表 `MANUAL_478_ON/OFF` 和 level hysteresis 不足以處理真球低端漂移或反相不穩。
  - 新增操作 log：每筆 report 記 raw、smRaw、baseline、delta、sign、span、level、phase、step、handCue 狀態。下一位接手者請先讀 log，再小步修正；不要再只改全域門檻。
