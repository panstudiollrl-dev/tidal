...
- **緊急：校正/呼吸偵測目前過度敏感（2026-07-15 Pan 實測）**：目前 `web/index.html` 最近一次把門檻降到 `HAND_CUE_PRESS_ON=0.16`、`HAND_CUE_RISE_DELTA=0.045` 後，出現「稍微碰到球就進下一階」的問題，甚至左手還沒真正完成三次、右手還沒測就跳走；30 秒「聽見呼吸」也會被輕觸誤判。請接手者優先修這個，不要直接相信現行狀態機。
