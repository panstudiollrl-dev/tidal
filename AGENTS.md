...
### 2026-08-06 (c) — Claude (Opus)｜Pan 回報：握力作答太敏感（輕輕碰就全滿、會滑到別區）＋ 4-7-8 改成規律數數（中英文版同步）

Pan：「我覺得很不錯 只是用握力回答各種問題 目前球都太敏感 輕輕碰就全滿 也很容易滑動去其他區域
但整個體驗是好的 現在我覺得 478 還是用規律數數跟震動就好 除了開頭那一下頌缽不要震動之外
每一下都要震動 可以再幫我改嗎」

兩件事，互相獨立。

**① 作答太敏感（`DESIGN.md` §6 新增一段「一段握壓怎麼變成一個答案」）**

- ⚠️ **接手的人請先讀這段再動門檻。** 這個抱怨最直覺的修法是「把門檻抬高」，而那是**錯的**：
  同一天早上的 (a) 那筆紀錄才剛把 `AFTER_ON` 從 460 raw 還原成 300 raw，為的是修 Pan 的
  另一個抱怨「有一顆球幾乎沒有效用」。抬回去就是把那個修法 revert 掉。
  **兩個抱怨都是真的，因為成因不同。**這次一個門檻都沒動。
- **實測成因**（用真的 `GripCalibrator` 重播 Pan 2026-07-22 的紀錄，只取 `raw`／`tMs`）：
  1. **峰值定案**：舊碼用 `a.peak`／`agreePeak` 定案。真球 dev p99 811(ball1)／2372(ball2)、
     max 1643／3103 ⇒ 一次「輕輕碰」裡本來就有幾幀尖峰，而峰值定案讓**任何一幀**決定整題。
     實測 **ball2 有 30% 的刻意握被記成 10/10**。
  2. **換級歸零**：`if(a.band !== band){ a.heldMs = 0 }`。實測一次握之中換級**中位 2 次、
     最多 18 次** ⇒ 使用者感覺「滑動去其他區域」，且 **ball2 43 次刻意握有 33 次湊不滿 1100ms**。
- **改法**：新增 `class AnswerSampler`（無 DOM／無時鐘依賴），取代兩處各自手寫的計分。
  滯後計時（分級變化不再打斷）＋跳過前 `AFTER_SETTLE_MS = 300`＋定案取**中位數**。
  ⚠️ 是中位數、**不是平均**：平均擋不住 3103 raw 那種尖峰（實測「380 raw ＋ 一次 3103」
  中位數 0.161 →「沒有」對，平均 0.217 →「有一點」錯）。我第一版 probe 用時間加權平均，
  **讓 Pan 的症狀更嚴重**，這才改成中位數。
- **結果**：13/13 個穩定力道的**分級與改版前完全相同**（弱球可達性一分不減），
  但「輕握 ＋ 一次尖峰」從貼頂變回「沒有」、ball2 貼頂率 30% → 0%。
  水位顯示改走 `display()`（＝目前的中位數），所以不會被一幀尖峰推到全滿——那就是 Pan 看到的「全滿」。
- 順帶收掉舊分岔：`AFTER_HOLD_MS`（zh 1100／en 900，本來就不一致）與 `AGREEMENT_FIX_MS`
  兩個常數都移除，兩頁現在只有 `AFTER_SAMPLE_MS = 1100` 一個數字。

**② 4-7-8 改成等速（`DESIGN.md` §7.2 改寫）**

- `BANGZI_CURVE` / `CURVE_MIN` / `CURVE_MAX` / `bangziGaps()` 全部移除，拍距一律
  `BANGZI_UNIT_MS`，從 880 → **1374**（維持一輪 26.1s／4 輪 104s，與 08-05 版**完全同長**）。
- ⚠️ **1374 取代了 Pan 自己 08-05 的 MIDI 決定**，所以 `midi_Accel_Rit_Rhythm.mid` 的分析
  結果沒有丟掉：完整保留在 `web/index.html` 那一節的註解、`DESIGN.md` §7.2、以及
  `tmp/sim_bangzi_478.js` 的檔頭（96 tpq、11 音、IOI `116 76 73 57 57 62 76 78 96 108` tick、
  力度 `127 70 63 52 57 63 65 61 57 50 39`）。要復原有據可循。
- 這**不違背** 2016 那篇 wearable device 的依據：該研究要的是「比使用者呼吸**慢**」這個
  速度**水準**，不是「速度會變」。等速 2.3 次／分照樣遠慢於靜息 12–16。漸快漸慢是美學選擇。
- 等速的附帶好處：段長比例變成**精確的 1.00 : 1.75 : 2.00**（段長＝`count × UNIT`）。
- **震動**：`if(!p.accent) sendHapticAll(...)` ⇒ 板（頌缽）只有聲音不震、眼全部都震。
  一輪 19 拍有 **16 拍會震**。⚠️ 只拿掉震動，**缽的聲音要留著**。
  `HAPTICS_ENABLED = false` 這條 guardrail 沒有被翻開，仍然只用顯式 `force` 且只綁時間表。
- 眼的強度仍**逐點遞減**（等速指的是拍距，不是力度；收尾的淡出留著）。

**測試（9 支對拍 1297 項、7 支變異 376/376）**

- 新增 `tmp/test_answer_sampler.js`（172 項）＋ `tmp/mutate_answer_sampler.js`（66/66）。
- ⚠️ **為什麼需要它**：Pan 回報這個症狀時，同一段程式碼**通過了 106 項斷言**——因為那些測試
  各自寫了一份自己的計分規則，頁面上真正在算分的那段沒有任何測試看著。所以四支相關測試
  現在一律用 regex 把頁面上**真正的** `AnswerSampler` 抽出來跑，不重寫一份。
- 修了四支被這次改版影響的測試：`sim_bangzi_478.js`（314，`[2]` 從「驗 MIDI 曲線」反轉成
  「驗精確等速＋曲線已移除」）、`mutate_bangzi_478.js`（86/86，新增 5 個震動閘門變異）、
  `test_survey_recovery.js`（123，原本會直接崩）、`mutate_survey_recovery.js`（60/60）、
  `test_grip_thresholds.js`（109）。
- 兩件測試品質上的判斷，都寫在原地：
  - `mutate_bangzi_478.js` 的「倒數改成算時間比例」在等速化之後成了**等價變異**
    （段長正好 `count × UNIT` ⇒ 比例式與拍點式完全重合，行為上無法區分）。沒有弱化斷言，
    而是換成兩個真的會錯位的形狀（分母漏掉段尾餘白、`floor`／`ceil` 寫反）。
  - `test_grip_thresholds.js [7]` 原本斷言「**所有**握壓段的中位數 ≥ 有一點」，那條問錯了問題：
    那 21／16 段包含整場所有路過的輕握，它們**本來就該**是「沒有」。舊的峰值規則把每段墊高
    約 0.025 才讓它剛好過關——**那個墊高正是 Pan 的抱怨本身**。改成驗可達性與比例
    （≥35% 達「有一點」、兩顆球都要摸得到「很明顯」、也都要表達得出低分）。

**要 Pan 決定的事（我沒有自己決定）**

1. `assets/hrir/` 確認是 ARI `nh2`、授權 **CC BY-SA 3.0 Unported**（不是 SADIE II、不是
   CC BY 4.0）。**share-alike 是個新的約束**，公開釋出前請 Pan 決定要不要接受。
2. 等速版是把 08-05 的漸快漸慢**整條移除**，不是調淡。若 Pan 想要「等速但保留一點起頭的
   留白」，那是另一個改法（現在的碼裡沒有任何速度變化的鉤子）。
3. zh 頁的 `trustedHeld`／`armAgreement`／`agreementHeld`／`agreeFloor` 這套壞球信任機制
   **仍然只有中文頁有**。要不要搬到 en 是 Pan 的決定（en 目前直接用 `Math.max(grip1, grip2)`，
   本來就沒有那個症狀）。

### 2026-08-06 (b) — Claude (Opus)｜Pan 回報：478 途中連線消失、呼吸結束後整個問卷卡住不能繼續（中英文版同步）

Pan：「478過程中因為沒有持續按壓握力球 連線就消失了 導致呼吸結束之後 整個問卷都不能繼續做而卡住」

- **做了什麼**：一個症狀、**三個各自獨立的頁面端缺陷**。每一個都足以單獨造成同一個畫面
  （一顆不動的球），而且畫面上完全看不出在等什麼。詳表見 `DESIGN.md` §7.0.2。
  1. **看門狗中途撤銷 WebHID 授權**（`ghost.forget()`）。`forget()` 撤銷的是**使用者授權**，
     而授權只能在**使用者手勢**裡重新取得；`syncBalls()` 走 `getDevices()`，只看得到還有
     授權的球 ⇒ 中途撤銷之後**剩下整段體驗都不可能自動接回**。改成只在「還沒開始體驗」
     （`state.phase === "before" && arrival.step === "connect"`）才撤銷，其餘保留授權並持續
     嘗試自動接回（並 log 一行，不要靜悄悄）。原本那行的正當用途（換球後舊授權佔住 slot，
     Pan 2026-07-17）只在還沒開始時才需要自動處理，而換球仍有手動的 R 鍵／`repairBalls()`。
  2. **問卷的開窗閘門（`!a.armed`）沒有任何逾時**。球凍在高位就永遠等不到「放開」。
     `arrival` 的同一個閘門 2026-07-29 就補過 `AGREEMENT_ARM_GRACE_MS`，**問卷漏了**。
     新增 `AFTER_ARM_GRACE_MS = 4000`，逾時就開窗但記下殘壓當地板。
  3. **空白鍵/Shift 後援的條件是 `!state.connected[slot]`**，而球靜掉之後 `connected` 還是
     true（看門狗要 6.5s 才清）⇒ 那段時間**連鍵盤退路都沒有**。新增
     `slotLive(slot)`（connected ＋ ready ＋ 最後一次 report 在 `HID_STALE_MS` 內），
     五處後援＋畫面提示都改用它；兩顆都不活時把退路寫在球上的字幕裡。
- **為什麼 4-7-8 是最容易踩到的一段**：自走化之後（Pan 2026-08-04「就不抓取使用者的捏握了」）
  使用者有整整 **104 秒**（26.1s × 4 輪）完全不需要出力——正好是球最容易進省電/靜默的一段。
  ②③ 一直都在，是 ①＋「104 秒不需出力」把它們一起引爆的。
- **現在能跑到哪 / 怎麼驗證**：新增 `tmp/test_survey_recovery.js`（**119 項**，兩頁各跑一次）
  ＋ `tmp/mutate_survey_recovery.js`（**56/56**）。既有 12 支測試全綠、7 支變異測試全綠
  （bangzi 82/82、grip_rezero 33/33、grip_thresholds 46/46、grip_trust 13/13、
  hrir_spatial 100/100、pebble_quality 32/32）。兩頁 `node --check` 都過。
- **未完成 / 卡住**：無。四項改動 zh／en 都已同步。
- **給下一位的四個教訓**（這次每一個都真的踩到了）：
  1. **「不卡住」不可以用「編一個分數」換來，而且做壞了測起來仍然是「問卷跑得完 4/4」**
     ——差別只在分數被記成什麼。我先寫錯了兩種形狀：**地板取開窗那一刻的瞬時值**
     （使用者正在試握就把答案本身記成殘壓 ⇒ 那一題永遠 0 分；實測殘壓 700raw ＋ 真的握到
     1400raw 記成 0），以及**回應窗仍從 `a.questionAt` 算**（4000ms 保險從 5200ms 窗裡扣
     ⇒ 只剩 1200ms 完成 1100ms 的 hold；實測反應 ≥1500ms 的人真的握了也記 0）。
     改法：地板取**等待期最低水位**且**只往下修**；窗口用 `max(questionAt, armedAt)`。
  2. **測試必須抽頁面裡真正的函式來跑，不要重寫一份 replica。** 診斷這題時我的 probe 自己
     重寫了邏輯、又用「不看提示就握」的握壓時程，量出來一整排 0 分**全是 probe 的假象**
     ——差點照著假象去改本來是對的程式碼。真函式＋假時鐘才問得出真話。
  3. **斷言「某個呼叫在不在」時要先把註解剝掉。** 這一段的註解本身就寫著 `ghost.forget()`
     與 `syncBalls()`（在解釋為什麼不呼叫前者），不剝的話**註解會替程式碼作證＝斷言恆真**
     （我這次連續踩到兩次，兩次都是變異測試抓出來的）。
  4. **任何「等使用者放開／等某個訊號」的閘門都必須有逾時。** 這已經是同一類 bug 的第二次
     （arrival 2026-07-29、問卷 2026-08-06）。以後新增這種閘門時，逾時保險與「逾時之後
     怎麼樣仍然拿得到真實答案」要一起設計，不是事後補。
- **順手修掉**：`tmp/_dbg.js` 的常數清單漏了 `GRIP_SETTLE_MS` / `GRIP_SETTLE_MIN_SHIFT`
  ⇒ 整支工具 ReferenceError（**在 `828951f` 就已經壞了，不是這次改出來的**，已驗證）。
- **`DESIGN.md` 已更新**：新增 §7.0.2「任何『等使用者放開』的閘門都必須有逾時」。
