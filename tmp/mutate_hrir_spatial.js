...
const MUTANTS = [
  ...
  // ── 引用義務（2026-08-05 來源查證確認後改寫）──────────────────────────────────
  // 舊的三個突變是在守「不要編引用」（當時來源未確認）。現在來源確認了，守的變成
  // 「引用必須完整而且正確」——每一件漏掉或寫錯都要被抓到。
  ["README 照抄 duck-hunt 的錯標示，說這批是 MeshRIR",
   null,
   r => r.replace(/不是 MeshRIR/g, "就是 MeshRIR").replace(/❌ duck-hunt/g, "✅ duck-hunt")],
  ["README 把 SADIE 寫回來源（文字線索指向它，但資料指向 ARI）",
   null,
   r => r.replace(/\*\*不是 SADIE\*\*/g, "**確認為 SADIE II（University of York）**")],
  ["README 只改掉其中一處說法（半套的錯字修訂也要抓到）",
   null,
   r => r.replace("❌ duck-hunt", "✅ duck-hunt")],
  ["README 授權寫成 CC BY 4.0（抄成 room.wav 那份的條款，漏掉 share-alike）",
   null,
   r => r.replace(/Creative Commons Attribution-\*\*ShareAlike\*\* 3\.0\n> Unported（CC BY-SA 3\.0）/,
                  "Creative Commons Attribution 4.0（CC BY 4.0）")],
  ["README 漏掉受測者編號 nh2（同一個資料庫有很多人，引用不完整）",
   null,
   r => r.replace(/，受測者 `nh2`/, "").replace(/nh2/g, "（某位受測者）")],
  ["README 漏掉資料集指定的論文引用（CC BY 的 attribution 沒滿足）",
   null,
   r => r.replace(/Majdak, P\., Goupell[\s\S]*?454–469\./,
                  "（引用略）")],
  ["README 只寫授權名稱、不提 share-alike 是新限制（會被當成跟 room.wav 一樣寬鬆）",
   null,
   r => r.replace(/⚠️ \*\*CC BY-SA 的 share-alike 條款是這個專案原本沒有考慮到的新限制\*\*[\s\S]*?見\n> `AGENTS\.md` 交接紀錄 2026-08-05 \(c\)。\n/, "")],
  ["README 留著查證前的「仍未確認」措辭（下一位會以為還沒查、白做一次）",
   null,
   r => r.replace(/## 來源（2026-08-05 已確認）與必須附上的標註/,
                  "## 來源與一個要修正的標示\n\n**但確切的原始資料集仍未確認**，`web/index.html` 的引用區塊目前寫「來源待確認」。")],
  ["頁面註解漏掉引用義務（README 不會跟著頁面一起被讀）",
   s => s.replace(/   ⚠️ 引用義務（2026-08-05 查證確認[\s\S]*?兩者授權不同。\n/, "")],
